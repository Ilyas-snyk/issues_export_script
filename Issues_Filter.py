...
def process_organization(org_path, original_file_pattern, filter_severities, filter_product,
    ...
    original_files = [f for f in os.listdir(org_path) if re.match(original_file_pattern, f, re.IGNORECASE)]
...
if __name__ == "__main__":
    ...
    logging.info(f"Found {len(org_dirs)} org folders: {org_dirs}")

    for org_id in org_dirs:
        logging.info(f"Processing org folder: {org_id}")
